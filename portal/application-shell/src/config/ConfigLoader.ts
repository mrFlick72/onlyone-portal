...
export const applicationConfigLoader = async () => {
    const configData = {
        ...
        analyticApiBaseUrl: import.meta.env.ANALYTIC_API_BASE_URL,
...
export async function getAnalyticApiBaseUrl() {
    const appConfig = await applicationConfigLoader()
    return appConfig.analyticApiBaseUrl;
}