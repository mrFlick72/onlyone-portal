...
**`config.ConfigurationManager` is a `sync.Once` singleton with no setter/reset.** `GetConfigurationManagerInstance()` reads `CONFIG_FILE_LOCATION` exactly once per process; there is no way to change a config value mid-test-run. When a `Configure()` step's behavior branches on a config value (e.g. `StandardMiddlewareConfigurer`'s health-check access-log skip, see ADR 0005), extract the branching logic into a small pure function that takes the resolved value as a parameter, so it can be unit-tested for both branches without touching the singleton.
