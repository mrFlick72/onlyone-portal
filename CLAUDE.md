...
- `cache.CacheProvider` has 3 methods, all context-aware: `GetContext(ctx, key) (string, bool)` / `SetContext(ctx, key, value) error` / `EvictContext(ctx, key) error`. There is no non-context variant — the original `Get`/`Set`/`Evict` were additive-era backward-compat shims, removed once an audit confirmed `budget-api` was the only real caller anywhere in the monorepo and had migrated off them (see `docs/adr/0004-remove-non-context-cache-provider-methods.md`).
- **Universal error contract**: a `CacheProvider` must never be the source of a caller-facing error. `GetContext` logs internal failures and returns `("", false)` — a miss, never an error. `SetContext`/`EvictContext` log internal failures and always return `nil`. Only the real delegate behind a cached call (e.g. a repository's DB/HTTP call on a cache miss) should ever surface an error to the application.
...
| budget-api   | POST       | `/api/budget/scheduled-expense`                  | Creates a scheduled expense (recurring template); Update/Delete/Pause-Resume land in follow-up tickets |
