...
func (p *recordingEventPublisher) CreateBudgetExpense(_ context.Context, e expense.BudgetExpense) error {
	p.created = append(p.created, e)
	return nil
}
func (p *recordingEventPublisher) UpdateBudgetExpense(context.Context, expense.BudgetExpense) error {
	return nil
}
func (p *recordingEventPublisher) DeleteBudgetExpense(context.Context, expense.BudgetExpense) error {
	return nil
}

// The Scheduled Expense generation engine (#55, ADR 0005) calls the real
// CreateBudgetExpense under an owner-only context — UserName, no AccessToken.
// Anything on the create path that reached tag-api would dereference the nil
// token; the tag repository mock has no expectations, so any call fails here.
func TestCreateBudgetExpenseUnderAnOwnerOnlyContextNeverResolvesTags(t *testing.T) {
	mockedBudgetExpenseIdProvider := new(DynamoDbBudgetExpenseIdProviderMock)
	createTagRepository := new(tags.SearchTagRepositoryMock)
	publisher := &recordingEventPublisher{}
	uut := &expense.CreateBudgetExpense{
		Repository:     newBudgetExpenseRepository(mockedBudgetExpenseIdProvider, createTagRepository),
		EventPublisher: publisher,
		Logger:         logging.GetLoggerInstanceForComponentByTypeName("owner-only-create-test"),
	}
	owner := "owner-only-user"
	ownerOnlyCtx := context.WithValue(context.Background(), "user", security.User{UserName: &owner})

	generated := expense.BudgetExpense{
		Date:   testutils.SafeDateFor("24/09/2026"),
	...
	mockedBudgetExpenseIdProvider.On("GenerateIdFor", &generated).Return("T1dORVJfT05MWQ==-T1dORVJfT05MWV9JRA==")

	err := uut.Execute(ownerOnlyCtx, &generated)

	assert.Equal(t, nil, err)
	createTagRepository.AssertNotCalled(t, "GetTagBy")
	assert.Equal(t, 1, len(publisher.created))
	assert.Equal(t, []tags.SearchTag{{Key: "housing", Value: "Housing"}}, publisher.created[0].Tags)

	readTagRepository := new(tags.SearchTagRepositoryMock)
	readTagRepository.On("GetTagBy", ownerOnlyCtx, "housing").Return(&tags.SearchTag{Key: "housing", Value: "Housing"}, nil)
	stored, err := newBudgetExpenseRepository(mockedBudgetExpenseIdProvider, readTagRepository).FindFor(ownerOnlyCtx, "T1dORVJfT05MWQ==-T1dORVJfT05MWV9JRA==")
	if err != nil || stored == nil {
		t.Fatalf("Expected the generated expense to be stored, got %+v / %v", stored, err)
	}
	assert.Equal(t, "owner-only-user", stored.UserName)
	assert.Equal(t, generated.Note, stored.Note)
}
